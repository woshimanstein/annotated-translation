...
MODEL_NAME = f'seq2seq_tagger_{BATCH_SIZE}'
log_file = open(os.path.join('model_weights', f'{MODEL_NAME}.log'), 'w')
print(f"training seq2seq_tagger with batch size {BATCH_SIZE} for {NUM_EPOCH} epochs")

# model setup
tokenizer = BartTokenizer.from_pretrained('facebook/bart-base')
model = Seq2SeqTagger(embed_size=EMBEDDING_DIM,
                hidden_size=HIDDEN_DIM,
                num_layers=NUM_LAYERS,
                dropout=DROPOUT,
                source_tagger=source_tagger).to(device)
